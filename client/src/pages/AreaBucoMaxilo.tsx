...
import { 
  ...
  CheckCircle, Play, User, ArrowRight, MapPin, Send, Loader2, Skull, ClipboardList, CheckSquare
} from "lucide-react";
import { Checkbox } from "@/components/ui/checkbox";

export default function AreaBucoMaxilo() {
  ...
  const calledPatients = bucoQueue?.filter(p => p.status === "called") || [];
  const inServicePatients = bucoQueue?.filter(p => p.status === "in_service") || [];
  ...
  const handleCallPatient = (entry: any) => {
    ...
    callPatient.mutate({
      ...
      professionalName: dentist?.name || "Buco-Maxilo",
    });
  };
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold tracking-tight flex items-center gap-2">
              ...
              Área Buco-Maxilo-Facial
            </h1>
            <p className="text-muted-foreground">Cirurgia e traumatologia buco-maxilo-facial</p>
          </div>
          ...
          <Card className="border-t-4 border-t-indigo-500">
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                ...
                <Skull className="h-8 w-8 text-indigo-500 opacity-20" />
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Pacientes sendo atendidos agora</CardDescription>
            </CardHeader>
            <CardContent>
              <ScrollArea className="h-[400px]">
                {/* Pacientes Chamados */}
        ...
        <Card>
          <CardHeader>
            <CardTitle>Procedimentos Buco-Maxilo-Faciais</CardTitle>
            <CardDescription>Procedimentos mais comuns desta especialidade</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                <Badge key={proc} variant="outline" className="justify-center py-2 text-sm">
                  {proc}
                </Badge>
              ))}
            </div>
