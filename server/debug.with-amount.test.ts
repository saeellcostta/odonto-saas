...
describe('Debug - Fluxo com amountToPay preenchido', () => {
  it('Encontrar atendimento com amountToPay e finalizar', async () => {
    ...
    // Buscar um atendimento com amountToPay > 0 e status pending_payment
    const entries = await database
      .select()
      .from(serviceQueue)
      .where((sq) => {
        // Buscar com amountToPay preenchido
        return sq.amountToPay !== null && sq.status === 'pending_payment';
      })
      .limit(1);

    if (entries.length === 0) {
      console.log('Nenhum atendimento com amountToPay encontrado');
      return;
    }
